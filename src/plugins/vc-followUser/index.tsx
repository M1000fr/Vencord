...
export const settings = definePluginSettings({
    ...
    onlyAlreadyInVoice: {
        type: OptionType.BOOLEAN,
        description: "Only follow if you are already in a voice channel",
        restartNeeded: false,
        default: false,
    },
});
...
function triggerFollow(userChannelId: string | null = getChannelId(settings.store.followUserId)) {
    if (settings.store.followUserId) {
        ...
        // Check if the user is in a voice channel when the setting is enabled
        if (settings.store.onlyAlreadyInVoice && !myChanId) {
            Toasts.show({
                message: "You must be in a voice channel to follow",
                id: Toasts.genId(),
